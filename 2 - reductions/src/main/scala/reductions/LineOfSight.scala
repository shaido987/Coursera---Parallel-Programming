...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    input.slice(from, until).zip(from to until - 1).map(x => x._1 / x._2).foldLeft(0.0f)(_ max _)
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    tree match {
	  ...
	  case n: Node => 
	  val (l, r) = parallel(downsweep(input, output, max(startingAngle, n.left.maxPrevious), n.left),
