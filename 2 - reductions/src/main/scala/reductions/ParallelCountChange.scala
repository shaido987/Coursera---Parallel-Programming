...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    (coins, money) match {
	  ...
	  case (_,c) if c < 0 => 0
	  case (Nil, _) => 0
	  case (h::t, c) if c >= h => countChange(c - h, h::t) + countChange(c, t)
	  case (h::t, c) => countChange(c, t)
	}
  }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) {
	  ...
	  (coins, money) match {
	    ...
	    case (Nil, _) => 0
	    case (h::t, c) if c >= h => {
		  val (l, r) = parallel(parCountChange(c - h, h::t, threshold), parCountChange(c, t, threshold))
		  l + r
		}
		case (h::t, c) => parCountChange(c, t, threshold)
 	  }
	}
  }
